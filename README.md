...
### 🚀 简单到极致
- **全栈一键起**：`docker compose up -d` 直接拉 Docker Hub 镜像起 proxy + collector + api + Postgres + Redis + MinIO
- **源码构建全栈**：`docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build`
- **开发依赖模式**：`docker compose -f docker-compose.deps.yml up -d` 只起 Postgres + Redis + MinIO，应用本机 `make run`
...
# 接入 DeepSeek / Grok / 本地 vLLM 只需要换掉 base_url 里的上游 URL 段：
...
- **任意上游都能进**：DeepSeek / Grok / Together / Moonshot / 本地 vLLM / Ollama / 自建 OpenAI 兼容服务，不需要在 AILens360 侧做任何 per-project 上游配置
...
当前推荐用 Docker Compose 部署。`docker-compose.yml` 默认拉取 Docker Hub 镜像 `coolbanhub/ailens360:latest`，一条命令启动 proxy、collector、api、Postgres、Redis、MinIO。

### 方式一：Docker Compose（推荐）

仓库根目录提供三份 compose 文件，按需选用：
...
```bash
make docker-up          # 拉发布镜像并启动全栈
make docker-down
make docker-build-up    # 从当前源码构建镜像并启动全栈
make docker-build-down
make docker-deps-up     # 仅启动 Postgres + Redis + MinIO
make docker-deps-down
```

### 方式二：二进制 / systemd 部署（高级）

适合已有外置 Postgres、Redis、MinIO，并希望用 systemd 管理进程的环境。当前仓库没有发布二进制附件，需要从源码构建。
