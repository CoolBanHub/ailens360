...
任何能被客户端写出完整 URL 的上游都能直接接入：DeepSeek、Grok、Together、Moonshot、本地 vLLM / Ollama、Anthropic、Gemini 官方端点等等，不需要在 AILens360 侧做任何 per-project 上游配置。
...
| 其他（`api.openai.com` / `api.deepseek.com` / `api.x.ai` / `localhost` / 任意自建） | `openai_parser`（默认） | OpenAI Chat Completions 兼容解析，业界事实标准 |

> 因为 OpenAI Chat Completions 已经是第三方上游的事实标准，所以"不匹配 anthropic / gemini host 一律按 openai 解析"足以覆盖绝大多数场景（DeepSeek、Grok、Together、Moonshot、本地 vLLM 都直接走 OpenAI 解析器）。
...
控制台 API 返回 Project 时会附带 `proxy_prefix = scheme://proxy-host[:port]`（proxy 进程的 origin，所有项目共用），以及一个 `example` 对象（包含 OpenAI / Anthropic / Gemini 三个常用上游的开箱即用 baseURL，并提供 `path_key` / `query_key` 两组带 `{project_key}` 占位符的 URL），方便前端直接展示或复制。当 `AILENS360_PUBLIC_URL` / `AILENS360_PROXY_PUBLIC_URL` 未配置时，api 进程会从请求头派生并自动替换为 proxy 监听端口（本地开发零配置就能工作）。
...
```json
{
  "code": 0,
  "data": {
    "models": ["gpt-5.5", "claude-3-5-sonnet"],
    "has_data": true
  }
}
```
  ...
  "data": {
    ...
    "example": {
      ...
      "path_key": {
        ...
        "anthropic": "http://localhost:8080/{project_key}/https://api.anthropic.com",
        "gemini":    "http://localhost:8080/{project_key}/https://generativelanguage.googleapis.com/v1beta"
      },
      "query_key": {
        "openai":    "http://localhost:8080/https://api.openai.com/v1?sk={project_key}",
        "anthropic": "http://localhost:8080/https://api.anthropic.com?sk={project_key}",
        "gemini":    "http://localhost:8080/https://generativelanguage.googleapis.com/v1beta?sk={project_key}"
      }
    },
...
> `proxy_prefix` 是 proxy 进程的 origin（所有项目共用，不是最终 baseURL）：使用时在它后面继续拼接完整的上游 URL（含 scheme），客户端 SDK 通常会在此基础上再追加自己的相对路径。`example` 字段给出了三个最常见上游的开箱即用形态，`path_key` / `query_key` 使用 `{project_key}` 占位符。项目身份可通过 `X-AILens-Project-Key: <project_key>`、`/<project_key>/` 路径前缀或 `?sk=<project_key>` 识别。
...
换成 DeepSeek / Grok / 本地 vLLM 只需要把上游 URL 段换掉，其它都不变。
